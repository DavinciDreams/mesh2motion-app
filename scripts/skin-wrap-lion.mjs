...
if (rig_path === undefined || visual_path === undefined || output_path === undefined) {
  console.error('Usage: node scripts/skin-wrap-lion.mjs <rig.glb> <visual.glb> <out.glb>')
  process.exit(1)
}
...
function build_grid (points) {
  ...
  const cell_size = Math.max(size.length() / 72, 0.0001)
  const cells = new Map()

  function cell_key_for_point (point) {
    const x = Math.floor((point.x - box.min.x) / cell_size)
    const y = Math.floor((point.y - box.min.y) / cell_size)
    const z = Math.floor((point.z - box.min.z) / cell_size)
    return [x, y, z]
  }
...
for (const target_mesh of visual_meshes(visual_gltf.scene)) {
  ...
  const skinned_mesh = new THREE.SkinnedMesh(geometry, material)
